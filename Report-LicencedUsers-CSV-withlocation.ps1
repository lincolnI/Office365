...
$ClientName = Read-Host -Prompt 'What Tenent is this for'
	...
	$Year = (Get-Date).Year
	$ReportName = ( "$Year" + "-" + "$Month" + "-" + "$Day" + "-" + "LicencedUsers-" + $ClientName)
	#>
...
$Users = Get-MsolUser -All | Where-Object { $_.isLicensed -eq "TRUE" } | select DisplayName, UserPrincipalName, Office | Sort-Object DisplayName
	...
	If (($Licenses).Count -gt 1)
	{
		Foreach ($License in $Licenses)
		{
			...
			If (!($TextLic))
			{
                ...
                $NewObject01 | Add-Member -MemberType NoteProperty -Name "Office" -Value $User.Office
