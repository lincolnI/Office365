...
ForEach ($client in $clients)
{
	...
	Write-Host "Working on $ClientName" -ForegroundColor $SystemMessageColor
		...
		If (($Licenses).Count -gt 1)
		{
			Foreach ($License in $Licenses)
			{
				...
				If (!($TextLic))
				{
					...
					$CSVName = Join-Path -Path $ReportPath -ChildPath "$ReportName-$ClientName.csv" 
					$NewObject02 = $null
					$NewObject02 = @()
