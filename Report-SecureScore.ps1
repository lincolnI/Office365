<#
    ...
    .Link 
    https://gcits.com/knowledge-base/export-customers-microsoft-secure-scores-to-csv-and-html-reports/?fbclid=IwAR2Cw_Z94MphZHuMicOMUkUo6xnJxNGQqYE8P-T3koQuf2TqGYLaC5wwlYw
...
#----------------------------------------------------------------
    ...
    $CompanyLogo = "http://www.relianceit.com.au/_assets/images/skins/reliance_logo_colour.png"
    $ClientName = Read-Host -Prompt 'What Tenent is this for'
        <#
        $Day = (Get-Date).Day
        $Month = (Get-Date).Month
        $Year = (Get-Date).Year
        $ReportName = ( "$Year" + "-" + "$Month" + "-" + "$Day" + "-" + "AllTenantOverview-" + $ClientName)
        #>
    $Date = Get-Date -Format "yyyy-MM-dd"
    $ReportName = ( "$Date" + "-" + "AllTenantOverview-" + $ClientName)
...
$yourLogo = "http://www.relianceit.com.au/_assets/images/skins/reliance_logo_colour.png"
...
$homePage = "https://docs.microsoft.com/en-us/office365/securitycompliance/office-365-secure-score"
$OnMicrosoftDomain = (Get-MsolDomain | Where-Object {$_.Name -Like "*onmicrosoft.com*"}).name
$appIdURI = "https://secure.$OnMicrosoftDomain/$((New-Guid).ToString())"
        ...
        do {
            try {
                ...
                if ($scores) {
                    ...
                    "$externalCSS <img class=`"float-right`" src=$yourLogo> $($completeReport -join "<p></p>")<img class=`"float-right`" src=$yourLogo><br><br><div>Report compiled by <strong>$tenantName</strong> on $((Get-Date).ToLongDateString())</div>" | Out-file $ReportPath\$($contract.DefaultDomainName).html
